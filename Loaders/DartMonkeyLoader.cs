...
public class DartMonkeyLoader : ModByteLoader<Il2CppAssets.Scripts.Models.Towers.TowerModel> {
	...
	protected override string BytesFileName => "DartMonkeys.bytes";
	...
	private void Set_v_MonkeyFanClubModel_Fields(int start, int count) {
		...
		var t = Il2CppType.Of<Il2CppAssets.Scripts.Models.Towers.Behaviors.Abilities.Behaviors.MonkeyFanClubModel>();
		var lifespanField = t.GetField("lifespan", bindFlags);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Behaviors.Abilities.Behaviors.MonkeyFanClubModel)m[i+start];
			lifespanField.SetValue(v,br.ReadSingle().ToIl2Cpp());
			...
			v.originDisplay = ModContent.CreatePrefabReference(br.ReadString());
			v.towerOriginDisplay = ModContent.CreatePrefabReference(br.ReadString());
			v.effectId = ModContent.CreatePrefabReference(br.ReadString());
			v.effectLeaderId = ModContent.CreatePrefabReference(br.ReadString());
			v.effectOnOtherId = ModContent.CreatePrefabReference(br.ReadString());
			v.bonusPierce = br.ReadSingle();
			v.projectileRadius = br.ReadSingle();
			v.bonusDamage = br.ReadInt32();
			v.effectModel = (Il2CppAssets.Scripts.Models.Effects.EffectModel) m[br.ReadInt32()];
			v.handBlurEjectEffectModel = (Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.EjectEffectModel) m[br.ReadInt32()];
			v.ejectX = br.ReadSingle();
			v.ejectY = br.ReadSingle();
			v.ejectZ = br.ReadSingle();
			v.otherDisplayModel = (Il2CppAssets.Scripts.Models.GenericBehaviors.DisplayModel) m[br.ReadInt32()];
			v.displayModel = (Il2CppAssets.Scripts.Models.GenericBehaviors.DisplayModel) m[br.ReadInt32()];
			v.leaderDisplayModel = (Il2CppAssets.Scripts.Models.GenericBehaviors.DisplayModel) m[br.ReadInt32()];
			v.endDisplayModel = (Il2CppAssets.Scripts.Models.Towers.Behaviors.CreateEffectAfterTimeModel) m[br.ReadInt32()];
	...
	private void Set_v_CreateEffectAfterTimeModel_Fields(int start, int count) {
		Set_v_TowerBehaviorModel_Fields(start, count);
		var t = Il2CppType.Of<Il2CppAssets.Scripts.Models.Towers.Behaviors.CreateEffectAfterTimeModel>();
		var lifespanField = t.GetField("lifespan", bindFlags);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Behaviors.CreateEffectAfterTimeModel)m[i+start];
			v.effectModel = (Il2CppAssets.Scripts.Models.Effects.EffectModel) m[br.ReadInt32()];
			lifespanField.SetValue(v,br.ReadSingle().ToIl2Cpp());
			v.useRoundTime = br.ReadBoolean();
		}
	...
	private void Set_v_ExpireProjectileAtScreenEdgeModel_Fields(int start, int count) {
		Set_v_ProjectileBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.ExpireProjectileAtScreenEdgeModel)m[i+start];
		}
	}
	...
	protected override Il2CppAssets.Scripts.Models.Towers.TowerModel Load(byte[] bytes) {
		using (var s = new MemoryStream(bytes)) {
			using (var reader = new BinaryReader(s)) {
				...
				Create_Records<Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.ExpireProjectileAtScreenEdgeModel>();
