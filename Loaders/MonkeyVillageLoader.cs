...
public class MonkeyVillageLoader : ModByteLoader<Assets.Scripts.Models.Towers.TowerModel> {
	...
	private void Set_v_TowerBehaviorWithOverlayModel_Fields(int start, int count) {
		Set_v_TowerBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Assets.Scripts.Models.Towers.TowerBehaviorWithOverlayModel)m[i+start];
	...
	private void Set_v_FilterInSetModel_Fields(int start, int count) {
		...
		for (var i=0; i<count; i++) {
			...
			v.towerSets = (Assets.Scripts.Models.TowerSets.TowerSet[]) m[br.ReadInt32()];
		}
	}
