...
public class SniperMonkeyLoader : ModByteLoader<Il2CppAssets.Scripts.Models.Towers.TowerModel> {
	...
	protected override string BytesFileName => "SniperMonkeys.bytes";
	...
	private void Set_v_CreateEffectFromCollisionToCollisionModel_Fields(int start, int count) {
		Set_v_ProjectileBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.CreateEffectFromCollisionToCollisionModel)m[i+start];
			v.effectModel = (Il2CppAssets.Scripts.Models.Effects.EffectModel) m[br.ReadInt32()];
	...
	private void Set_v_CheckTargetsWithoutOffsetsModel_Fields(int start, int count) {
		Set_v_AttackBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Behaviors.Attack.Behaviors.CheckTargetsWithoutOffsetsModel)m[i+start];
		}
	}
	...
	private void Set_v_LeakDangerAttackSpeedModel_Fields(int start, int count) {
		Set_v_WeaponBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.LeakDangerAttackSpeedModel)m[i+start];
	...
	private void Set_v_TargetSupplierSupportModel_Fields(int start, int count) {
		...
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Behaviors.TargetSupplierSupportModel)m[i+start];
			v.mutatorId = br.ReadBoolean() ? null : br.ReadString();
			v.targetSupplier = (Il2CppAssets.Scripts.Models.Towers.Behaviors.Attack.Behaviors.TargetSupplierModel) m[br.ReadInt32()];
		}
	}
	...
	private void Set_v_SlowMaimMoabModel_Fields(int start, int count) {
		...
		var t = Il2CppType.Of<Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.SlowMaimMoabModel>();
		var moabDurationField = t.GetField("moabDuration", bindFlags);
		var bfbDurationField = t.GetField("bfbDuration", bindFlags);
		var zomgDurationField = t.GetField("zomgDuration", bindFlags);
		var ddtDurationField = t.GetField("ddtDuration", bindFlags);
		var badDurationField = t.GetField("badDuration", bindFlags);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.SlowMaimMoabModel)m[i+start];
			moabDurationField.SetValue(v,br.ReadSingle().ToIl2Cpp());
			bfbDurationField.SetValue(v,br.ReadSingle().ToIl2Cpp());
	...
	protected override Il2CppAssets.Scripts.Models.Towers.TowerModel Load(byte[] bytes) {
		using (var s = new MemoryStream(bytes)) {
			using (var reader = new BinaryReader(s)) {
				...
				Create_Records<Il2CppAssets.Scripts.Models.Towers.Projectiles.Behaviors.CreateEffectFromCollisionToCollisionModel>();
				...
				Create_Records<Il2CppAssets.Scripts.Models.Towers.Behaviors.Attack.Behaviors.CheckTargetsWithoutOffsetsModel>();
				...
				Create_Records<Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.LeakDangerAttackSpeedModel>();
