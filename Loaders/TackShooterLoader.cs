...
public class TackShooterLoader : ModByteLoader<Il2CppAssets.Scripts.Models.Towers.TowerModel> {
	...
	protected override string BytesFileName => "TackShooters.bytes";
	...
	private void Set_v_SpinModel_Fields(int start, int count) {
		...
		var t = Il2CppType.Of<Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.SpinModel>();
		var rotationPerSecondField = t.GetField("rotationPerSecond", bindFlags);
		for (var i=0; i<count; i++) {
			var v = (Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.SpinModel)m[i+start];
			rotationPerSecondField.SetValue(v,br.ReadSingle().ToIl2Cpp());
		}
	}
	...
	protected override Il2CppAssets.Scripts.Models.Towers.TowerModel Load(byte[] bytes) {
		using (var s = new MemoryStream(bytes)) {
			using (var reader = new BinaryReader(s)) {
				...
				Create_Records<Il2CppAssets.Scripts.Models.Towers.Weapons.Behaviors.SpinModel>();
