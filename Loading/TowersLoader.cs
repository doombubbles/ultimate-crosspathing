...
public class TowersLoader {
	...
	private void Set_v_RemoveBloonModifiersModel_Fields(int start, int count) {
		...
		for (var i=0; i<count; i++) {
			...
			v.hasBeenModded = br.ReadBoolean();
			v.whitelistTagModFilter = (List<System.String>) m[br.ReadInt32()];
			v.blacklistTagModFilter = (List<System.String>) m[br.ReadInt32()];
		}
	}
	...
	private void Set_v_MonkeyFanClubModel_Fields(int start, int count) {
		...
		for (var i=0; i<count; i++) {
			...
			v.effectModel = (Assets.Scripts.Models.Effects.EffectModel) m[br.ReadInt32()];
			v.handBlurEjectEffectModel = (Assets.Scripts.Models.Towers.Weapons.Behaviors.EjectEffectModel) m[br.ReadInt32()];
			v.ejectX = br.ReadSingle();
			v.ejectY = br.ReadSingle();
			v.ejectZ = br.ReadSingle();
	...
	private void Set_v_CreateEffectWhileAttackingModel_Fields(int start, int count) {
		Set_v_AttackBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Assets.Scripts.Models.Towers.Behaviors.Attack.Behaviors.CreateEffectWhileAttackingModel)m[i+start];
			v.effectModel = (Assets.Scripts.Models.Effects.EffectModel) m[br.ReadInt32()];
	...
	private void Set_v_ActivateAttackSwitchModel_Fields(int start, int count) {
		Set_v_AbilityBehaviorModel_Fields(start, count);
		for (var i=0; i<count; i++) {
			var v = (Assets.Scripts.Models.Towers.Behaviors.Abilities.Behaviors.ActivateAttackSwitchModel)m[i+start];
			v.lifespanFrames = br.ReadInt32();
			v.firstAttack = (Assets.Scripts.Models.Towers.Behaviors.Attack.AttackModel) m[br.ReadInt32()];
			v.secondAttack = (Assets.Scripts.Models.Towers.Behaviors.Attack.AttackModel) m[br.ReadInt32()];
			v.processOnActivate = br.ReadBoolean();
		}
	}
	...
	public Assets.Scripts.Models.Towers.TowerModel Load(byte[] bytes) {
		using (var s = new MemoryStream(bytes)) {
			using (var reader = new BinaryReader(s)) {
				...
				Create_Records<Assets.Scripts.Models.Towers.Behaviors.Abilities.Behaviors.ActivateAttackSwitchModel>();
				...
				Set_v_ActivateAttackSwitchModel_Fields(br.ReadInt32(), br.ReadInt32());
